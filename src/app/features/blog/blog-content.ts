...
export interface DiagnosticReportData {
  readonly codebaseStats: {
    readonly totalLines: number;
    readonly totalFiles: number;
    readonly languages: ReadonlyArray<{ name: string; percentage: number; color: string }>;
  };
  readonly complexityHeatmap: ReadonlyArray<{
    module: string;
    complexity: number;
  ...
  readonly dependencyGraph: ReadonlyArray<{
    package: string;
    version: string;
    status: 'current' | 'outdated' | 'deprecated' | 'vulnerable';
    dependents: number;
    severity?: 'low' | 'medium' | 'high' | 'critical';
  }>;
  readonly aiArtifacts: ReadonlyArray<{
    file: string;
  ...
  readonly testCoverage: {
    overall: number;
    byModule: ReadonlyArray<{ module: string; coverage: number }>;
  };
  readonly technicalDebtScore: {
    overall: number;
    breakdown: ReadonlyArray<{ category: string; score: number; maxScore: number }>;
  };
}
...
export interface CaseStudy {
  ...
  readonly keywords: ReadonlyArray<string>;
  readonly summary: string;
  readonly diagnosticReport?: DiagnosticReportData;
}
...
export const CASE_STUDIES: Record<string, CaseStudy> = {
  ...
  'sample-diagnostic-report': {
    slug: 'sample-diagnostic-report',
    title: 'Deep Codebase Review: Sample Diagnostic Report',
    client: 'TechCorp Industries',
    industry: 'Enterprise SaaS',
    stack: ['TypeScript', 'React', 'Node.js', 'MongoDB', 'AWS'],
    challenge:
    ...
    metrics: [
      { metric: '23', label: 'critical security vulnerabilities identified in dependency chain.' },
      { metric: '67', label: 'AI-generated code artifacts flagged for review and hardening.' },
      { metric: '156', label: 'code duplication instances mapped for consolidation.' },
      { metric: '18%', label: 'potential bundle size reduction through deduplication.' },
    ],
    keywords: [
    ...
    diagnosticReport: {
      codebaseStats: {
        ...
        languages: [
          ...
          { name: 'CSS/SCSS', percentage: 8, color: '#cc6699' },
          { name: 'HTML', percentage: 4, color: '#e34f26' },
          { name: 'Other', percentage: 2, color: '#6b7280' },
        ],
      },
      complexityHeatmap: [
        { module: 'src/services/auth', complexity: 87, risk: 'critical', linesOfCode: 3421 },
        { module: 'src/components/Dashboard', complexity: 72, risk: 'high', linesOfCode: 2893 },
        { module: 'src/api/payments', complexity: 68, risk: 'high', linesOfCode: 2156 },
        { module: 'src/utils/validation', complexity: 54, risk: 'medium', linesOfCode: 1834 },
        { module: 'src/hooks/useData', complexity: 51, risk: 'medium', linesOfCode: 1622 },
        { module: 'src/components/Forms', complexity: 47, risk: 'medium', linesOfCode: 2441 },
        { module: 'src/services/analytics', complexity: 38, risk: 'low', linesOfCode: 1288 },
        { module: 'src/components/Charts', complexity: 35, risk: 'low', linesOfCode: 1967 },
      ],
      dependencyGraph: [
        { package: 'lodash', version: '4.17.15', status: 'vulnerable', dependents: 34, severity: 'critical' },
        { package: 'moment', version: '2.24.0', status: 'deprecated', dependents: 28, severity: 'medium' },
        { package: 'axios', version: '0.21.1', status: 'vulnerable', dependents: 52, severity: 'high' },
        { package: 'react-router', version: '5.2.0', status: 'outdated', dependents: 67, severity: 'low' },
        { package: 'uuid', version: '3.4.0', status: 'outdated', dependents: 23, severity: 'low' },
        { package: 'express', version: '4.16.4', status: 'vulnerable', dependents: 41, severity: 'high' },
        { package: 'jsonwebtoken', version: '8.5.1', status: 'current', dependents: 19 },
        { package: 'mongoose', version: '6.8.0', status: 'current', dependents: 33 },
      ],
      aiArtifacts: [
        {
          file: 'src/utils/dataProcessor.ts',
          issues: ['No error handling', 'Missing input validation', 'Untested edge cases'],
          confidence: 94,
        },
        {
          file: 'src/api/userController.ts',
          issues: ['Inconsistent error responses', 'No rate limiting', 'SQL injection risk'],
          confidence: 89,
        },
        {
          file: 'src/services/emailService.ts',
          issues: ['Hardcoded values', 'No retry logic', 'Missing logging'],
          confidence: 87,
        },
        {
          file: 'src/hooks/useAuth.ts',
          issues: ['Race conditions', 'Memory leaks possible', 'No cleanup'],
          confidence: 91,
        },
      ],
      testCoverage: {
        overall: 42,
        byModule: [
          { module: 'Authentication', coverage: 78 },
          { module: 'Payment Processing', coverage: 65 },
          { module: 'User Management', coverage: 54 },
          { module: 'Dashboard Components', coverage: 38 },
          { module: 'Data Services', coverage: 31 },
          { module: 'Utilities', coverage: 22 },
          { module: 'API Controllers', coverage: 19 },
        ],
      },
      technicalDebtScore: {
        overall: 68,
        breakdown: [
          { category: 'Code Complexity', score: 72, maxScore: 100 },
          { category: 'Test Coverage', score: 42, maxScore: 100 },
          { category: 'Dependency Health', score: 54, maxScore: 100 },
          { category: 'Documentation', score: 38, maxScore: 100 },
          { category: 'Security Posture', score: 61, maxScore: 100 },
          { category: 'Performance', score: 75, maxScore: 100 },
        ],
      },
    },
  },
